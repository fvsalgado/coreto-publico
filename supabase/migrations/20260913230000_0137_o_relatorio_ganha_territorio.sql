-- 0137 — O relatório mensal passa a ter território: juntas ligadas e freguesias.
--
-- «26 das 84 juntas do Médio Tejo já publicam na agenda regional» é a frase que
-- o presidente da CIM diz no relatório anual, e até aqui não se calculava: o
-- numerador só saía casando prefixos de id, e o denominador não existia na base.
-- A 0135 deu-lhe o `parish_site` e a 0136 deu-lhe o `parish_count`; falta o
-- sítio onde a fração é lida, que é este.
--
-- **Contagens, e não a percentagem.** O plano pedia a percentagem; o
-- `apps/web/src/lib/admin/relatorio.ts` tem escrito, em comentário, que os
-- números vão como contagens e nunca como percentagens, porque «a percentagem é
-- uma leitura, e quem recebe o ficheiro faz a sua». A regra da casa ganha ao
-- plano: vão as duas contagens e o denominador, e quem divide é quem lê.
--
-- **`parishes` a nulo quando falta um concelho.** Somar só os concelhos que têm
-- as freguesias contadas dava um denominador menor sem ninguém escrever nada de
-- falso — que é a mentira mais barata que há numa fração. Nulo é «não consegui
-- saber», e é diferente de zero. As schema-checks já exigem o denominador a
-- qualquer região que se declare completa (0136); isto é a cintura para uma
-- região ainda a nascer.
--
-- A função é reescrita por inteiro porque é o que um `create or replace` de
-- plpgsql obriga; o que muda é uma variável, uma secção e uma chave.
...
declare
  v jsonb;
begin
  v := public.monthly_report('medio-tejo', current_date) -> 'territory';

  assert v is not null, 'o relatório mensal não traz a secção do território';

  assert (v ->> 'municipalities')::integer =
         (select count(*) from public.municipalities where region_id = 'medio-tejo'),
    'a contagem de concelhos do território não bate com a tabela';

  assert (v ->> 'parishes')::integer =
         (select sum(parish_count) from public.municipalities where region_id = 'medio-tejo'),
    'o denominador das freguesias não bate com a soma dos concelhos';

  assert (v ->> 'parish_sources_enabled')::integer =
         (select count(*) from public.sources s
            join public.municipalities m on m.id = s.municipality_id
           where m.region_id = 'medio-tejo' and s.kind = 'parish_site' and s.is_enabled),
    'o numerador das juntas ligadas não bate com as fontes';

  -- E a prova que importa mais: um concelho sem freguesias contadas faz o
  -- denominador desaparecer, em vez de o encolher em silêncio. Numa transação
  -- que se desfaz.
  update public.municipalities set parish_count = null where id = 'tomar';
  v := public.monthly_report('medio-tejo', current_date) -> 'territory';
  assert v -> 'parishes' = 'null'::jsonb,
    format('com um concelho por contar, o denominador devia ser nulo e foi %s', v ->> 'parishes');
