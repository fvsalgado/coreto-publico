-- 0150 — O relatório leva os cinco compromissos.
--
-- A 0149 escreveu `report_promises`; esta migração é a que a põe no
-- relatório, na cauda do objeto, para que nada do que já lê por nome parta.
--
-- **O bloco tem base própria, e é o ponto todo.** Os cinco indicadores contam
-- o que foi **programado** — canónico, nem rascunho nem escondido, com a
-- programação a tocar o mês —, enquanto `comparison.current.events_happening`
-- conta só `status = 'published'`. Em setembro de 2026 são 104 e 101, e em
-- abril são 13 e 5. Quem somar os dois números chega a um terceiro que não
-- quer dizer nada; a ficha técnica de `/indicadores` diz-lhe porquê antes de
-- ele tentar.
--
-- A função é refeita por inteiro: o `create or replace` de uma função em
-- plpgsql substitui o corpo todo. O texto vem da 0147.
...
begin
  ...
  -- ---- Os compromissos ----
  --
  -- Cinco famílias, numa base própria: **programado**, e não «publicado
  -- hoje». Ver a 0149 — contar só `published` faz o CAMINHOS, que uniu dez
  -- concelhos em abril e maio, desaparecer do relatório do mês em que
  -- aconteceu, por os seus eventos terem sido arquivados desde então.
  --
  -- É por isso que `promises.total` não é o `comparison.current.
  -- events_happening`: em setembro de 2026 são 104 e 101. A diferença está
  -- escrita na ficha técnica de `/indicadores`, e é uma diferença de
  -- pergunta, não de contagem.
  v_promessas := public.report_promises(p_region, v_inicio, v_seguinte);
...
declare
  v_rel  jsonb;
  v_mes  date;
  v_abr  date := date '2026-04-01';
begin
  v_mes := date_trunc('month', current_date)::date;
  v_rel := public.monthly_report('medio-tejo', v_mes);

  if v_rel -> 'promises' = 'null'::jsonb or v_rel -> 'promises' is null then
    raise exception 'o relatório veio sem o bloco dos compromissos';
  end if;

  -- O bloco do relatório e a função chamada à parte dão o mesmo. Se um dia
  -- divergirem, é porque alguém passou outra janela — e é aqui que se sabe.
  if v_rel -> 'promises' <> public.report_promises('medio-tejo', v_mes,
       (v_mes + interval '1 month')::date) then
    raise exception 'o bloco dos compromissos não é o que a função devolve para o mesmo mês';
  end if;

  -- A base é «programado» e não «publicado», e a diferença tem de ser
  -- visível: o total dos compromissos nunca é menor do que os eventos a
  -- decorrer, porque contém os arquivados por cima deles.
  if (v_rel -> 'promises' ->> 'total')::bigint
     < (v_rel -> 'comparison' -> 'current' ->> 'events_happening')::bigint then
    raise exception 'os compromissos contam % e os eventos a decorrer %: a base encolheu',
      v_rel -> 'promises' ->> 'total',
      v_rel -> 'comparison' -> 'current' ->> 'events_happening';
  end if;

  -- E a janela do bloco é a do mês pedido, e não a de hoje.
  if (v_rel -> 'promises' ->> 'from')::date <> v_mes then
    raise exception 'os compromissos vieram da janela % e o mês é %',
      v_rel -> 'promises' ->> 'from', v_mes;
  end if;

  v_rel := public.monthly_report('medio-tejo', v_abr);
  if (v_rel -> 'promises' ->> 'from')::date <> v_abr then
    raise exception 'o relatório de abril trouxe os compromissos de outro mês';
  end if;
end
$$;
